...
public plugin_init( )
{
	register_plugin( "Golden AK47", "1.0", "Supremache" );
	register_cvar("GoldenAK47", "1.0", FCVAR_SERVER | FCVAR_SPONLY | FCVAR_UNLOGGED );
...
public fw_CmdStart(id, uc_handle, seed)
{
	...
	if( ( pev( id, pev_oldbuttons ) & IN_ATTACK ) )
	{
		...
		if( !iClip || iWeapon != get_weaponid( GOLDENAK_WEAPON ) || !is_entity( iWeaponEnt ) )
			return HAM_IGNORED
